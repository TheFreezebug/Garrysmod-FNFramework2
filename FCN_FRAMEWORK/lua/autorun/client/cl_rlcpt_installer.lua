...
concommand.Add("update_magspack",function()
    LocalPlayer():ChatPrint("Downloading magspak.fst")
    http.Fetch("http://fcnrailways.site.nfoservers.com/magspack/magpak.fst",function(body) 
        file.Write("magspak.dat",body)
        LocalPlayer():ChatPrint("Got magspak.fst")
        LocalPlayer():ChatPrint("Opening magspak.fst")
        local rptfst = fst.OpenFST("magspak.dat")
        print(rptfst)
        local rtab = rptfst:GetFileTable()
        file.CreateDir("expression2/_magspack_")
        for i,fi in pairs(rtab) do
            local sts,cnt = rptfst:ReadFile(fi)
            wpath("expression2/_magspack_" .. fi,cnt)
        end       
        LocalPlayer():ChatPrint("Awesome! Mag's Pack is installed! Open your e2 tool and be sure to click the 'Update' button at the bottom. Look for the _magspack_ folder.")
    end ,function(erro)
        print("Problem getting the fst file.")
        print(erro)
 end)     
