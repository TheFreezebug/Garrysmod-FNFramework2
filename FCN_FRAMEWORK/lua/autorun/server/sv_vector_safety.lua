...
local MOVETYPE_PUSH=MOVETYPE_PUSH
...
local function vector_clamp(vec,len)
	...
	vec.x=x>len and len or x<nlen and nlen or x~=x and 0 or x
...
local MAX_SAFE_LEN_SQ = MAX_SAFE_LEN*MAX_SAFE_LEN

local PhysObj_SetPos=PhysObj.SetPos
function PhysObj:SetPos(vec)
	local len = vec:LengthSqr()
	if len>MAX_SAFE_LEN_SQ or len~=len then
		vector_safety(vec)
	end
...
function PhysObj:RotateAroundAxis(vec,ang)
	...
	local len = vec:LengthSqr()
	if len>999999999999 or len~=len then
